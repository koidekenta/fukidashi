...
<hr>
	<ol class="fukidashi-header-menu">
		<span class="fukidashi-header-menu-item">ふきだし</span><span class="fukidashi-header-menu-item"><a href="/users/reply/<?= h($user_info["username"]) ?>">返信</a></span><span class="fukidashi-header-menu-item"><a href="/users/media/<?= h($user_info["username"]) ?>">メディア</a></span>
	</ol>
...
<?php if(!empty($koteis)){ ?>
	<ol id="kotei-fukidashi">
		            <li class="item" style="position:relative;border-top:1px solid #E3E0DE;padding:15px;font-size:14px;" data-toggle="modal" data-target="#exampleModalCenter3">
		<div data-created="<?php echo $koteis["created"]->format("Y-m-d H:i:s"); ?>" data-slug="<?= h($koteis["slug"]) ?>" data-username="<?= h($koteis["user"]["username"]) ?>" data-id="<?= h($koteis["id"]) ?>" data-post="<?= h($koteis["post"]) ?>" data-imageurl="<?php echo App\Utils\AppUtility::im_ch(h($koteis["user"]["imageurl"])); ?>" data-type="kotei-kaijyo">
			<div style="display:flex;align-items:center;"><div><i class="fas fa-filter my-small" style="color:red;"></i></div><div style="margin-left:5px;font-size:12px;">固定された吹き出し</div></div>
			<div class="fukidashi-header">
				<div><a href="/users/view/<?= h($koteis["user"]["username"]) ?>"><?= h($koteis["user"]["username"]) ?></a>　</div>
				<div style="margin-left:10px;"><a href="/users/view/<?= h($koteis["user"]["username"]) ?>/<?= h($koteis["slug"]) ?>"><?php echo App\Utils\AppUtility::time_change($koteis["created"]); ?></a> </div>
	...
	<div class="body" style="z-index:1000;display:none;overflow:hidden;font-size:13px;width:200px;margin-top:20px;position:absolute;right:10px;background-color:white;border:solid 1px #CBCDD2;padding:5px 0px;border-radius:5px;">
		<ul class="toggle-item">
			<li class="toggle-item-list"><a class="kotei-kaijyo">このふきだしの固定を解除する</a></li>
			...
			<?php echo $this->Html->image(App\Utils\AppUtility::im_ch($koteis["user"]["imageurl"]),["width" => 40, "height" => 40, "class" => "usericon"]); ?>
			<div class="article-body">
			<?php echo App\Utils\AppUtility::pc(h($koteis["post"])); ?>
			
			<?php
				if($koteis["post_img"]){
					echo '<div class="img_container">';
					echo $this->Html->image($koteis["post_img"], ['class' => 'header-image']);
					echo '</div>';
				}
			?>
			
				<div class="flex">
					<p class="fukidashi-footer"><label class="fukidashi-footer-comment"><i class="far fa-comment mysize"><input type="button" class="comment" style="display:none;"></i></label><span class="comment_num"><?= $koteis["comment_num"] ?></span></p>
					<p class="fukidashi-footer"><label class="fukidashi-footer-diffusion"><?php
								if($koteis["my_retweet_true"] !== null){
									echo '<i class="fas fa-recycle recycle-clicked">';
								}else{
									echo '<i class="fas fa-recycle mysize">';
								}
							?><input type="button" class="diffusion" style="display:none;"  data-toggle="modal" data-target="#exampleModalCenter2"></i></label><span class="diffusion_num"><?= $koteis["refukidashi_num"] ?></span></p>
					<p class="fukidashi-footer"><label class="fukidashi-footer-favorite"><?php
								if($koteis["my_favorite_true"] != null){
									 echo '<i class="fas fa-heart heart-clicked">';
								}else{
									 echo '<i class="far fa-heart mysize">';
								}
							?><input type="button" class="favorite" style="display:none;"></i></label><span class="favorite_num"><?= $koteis["favorite_num"] ?></span></p>
